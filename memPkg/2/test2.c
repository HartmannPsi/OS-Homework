...
int main() {
  ...
  for (int i = 0; i < THREAD_PRODUCERS; i++) pthread_join(producers[i], NULL);
  for (int i = 0; i < THREAD_CONSUMERS; i++) pthread_join(consumers[i], NULL);

  pthread_mutex_destroy(&lock);
  printf(
      "Multi-threaded test completed. Verify sequential consistency manually "
      "(check Timestamps).\n");
  return 0;
}
