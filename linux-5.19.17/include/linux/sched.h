...
struct task_struct {
   ...
   * //true deactivate_task()		      ttwu_queue_wakelist()) p->on_rq =
...
static inline unsigned int __task_state_index(unsigned int tsk_state,
  ...
  if (tsk_state == TASK_IDLE) state = TASK_REPORT_IDLE;
   ...
   */
  if (tsk_state == TASK_RTLOCK_WAIT) state = TASK_UNINTERRUPTIBLE;
...
#define PF_MEMALLOC_PIN                                                       \
  0x10000000 /* Allocation context constrained to zones which allow long term \
                pinning. */
...
static inline int _cond_resched(void) { return __cond_resched(); }
...
static inline bool vcpu_is_preempted(int cpu) { return false; }
#endif
...
static inline void rseq_set_notify_resume(struct task_struct *t) {
  if (t->rseq) set_tsk_thread_flag(t, TIF_NOTIFY_RESUME);
}
...
static inline void rseq_handle_notify_resume(struct ksignal *ksig,
                                             struct pt_regs *regs) {
  if (current->rseq) __rseq_handle_notify_resume(ksig, regs);
}
