...
static void check_sync_rss_stat(struct task_struct *task) {
  if (unlikely(task != current)) return;
...
void free_pgd_range(struct mmu_gather *tlb, unsigned long addr,
  ...
  if (end - 1 > ceiling - 1) end -= PMD_SIZE;
...
static inline void add_mm_rss_vec(struct mm_struct *mm, int *rss) {
  ...
  if (current->mm == mm) sync_mm_rss(mm);
  for (i = 0; i < NR_MM_COUNTERS; i++)
    if (rss[i]) add_mm_counter(mm, i, rss[i]);
}
...
static void print_bad_pte(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (nr_shown++ == 0) resume = jiffies + 60 * HZ;
  ...
  if (page) dump_page(page, "bad pte");
...
struct page *vm_normal_page(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (unlikely(vma->vm_flags & (VM_PFNMAP | VM_MIXEDMAP))) {
    if (vma->vm_flags & VM_MIXEDMAP) {
      ...
      if (pfn == vma->vm_pgoff + off) return NULL;
      if (!is_cow_mapping(vma->vm_flags)) return NULL;
    }
  }
...
struct page *vm_normal_page_pmd(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (pmd_devmap(pmd)) return NULL;
  if (is_huge_zero_pmd(pmd)) return NULL;
  if (unlikely(pfn > highest_memmap_pfn)) return NULL;
...
static unsigned long copy_nonpresent_pte(struct mm_struct *dst_mm,
  ...
  if (likely(!non_swap_entry(entry))) {
    if (swap_duplicate(entry) < 0) return -EIO;
     ...
     */
    if (is_writable_device_private_entry(entry) && is_cow_mapping(vm_flags)) {
    ...
    if (try_restore_exclusive_pte(src_pte, src_vma, addr)) return -EBUSY;
    return -ENOENT;
  ...
  if (!userfaultfd_wp(dst_vma)) pte = pte_swp_clear_uffd_wp(pte);
  set_pte_at(dst_mm, addr, dst_pte, pte);
  return 0;
}
...
static inline int copy_present_pte(struct vm_area_struct *dst_vma,
  ...
  if (page && PageAnon(page)) {
    ...
    if (unlikely(page_try_dup_anon_rmap(page, false, src_vma))) {
      ...
      put_page(page);
      return copy_present_page(dst_vma, src_vma, dst_pte, src_pte, addr, rss,
                               prealloc, page);
    }
    rss[mm_counter(page)]++;
  } else if (page) {
  ...
  if (!userfaultfd_wp(dst_vma)) pte = pte_clear_uffd_wp(pte);

  set_pte_at(dst_vma->vm_mm, addr, dst_pte, pte);
  return 0;
}

static inline struct page *page_copy_prealloc(struct mm_struct *src_mm,
...
again:
  ...
  do {
    ...
    if (unlikely(!pte_present(*src_pte))) {
      ret = copy_nonpresent_pte(dst_mm, src_mm, dst_pte, src_pte, dst_vma,
                                src_vma, addr, rss);
      if (ret == -EIO) {
    ...
    ret = copy_present_pte(dst_vma, src_vma, dst_pte, src_pte, addr, rss,
                           &prealloc);
    /*
...
static inline int copy_pmd_range(struct vm_area_struct *dst_vma,
  ...
  do {
    ...
    if (is_swap_pmd(*src_pmd) || pmd_trans_huge(*src_pmd) ||
      ...
      err = copy_huge_pmd(dst_mm, src_mm, dst_pmd, src_pmd, addr, dst_vma,
                          src_vma);
      if (err == -ENOMEM) return -ENOMEM;
      if (!err) continue;
      /* fall through */
    }
    if (pmd_none_or_clear_bad(src_pmd)) continue;
...
static inline int copy_pud_range(struct vm_area_struct *dst_vma,
  ...
  do {
    ...
    if (pud_none_or_clear_bad(src_pud)) continue;
...
static inline int copy_p4d_range(struct vm_area_struct *dst_vma,
  ...
  do {
    next = p4d_addr_end(addr, end);
    if (p4d_none_or_clear_bad(src_p4d)) continue;
...
static bool vma_needs_copy(struct vm_area_struct *dst_vma,
   ...
   */
  if (userfaultfd_wp(dst_vma)) return true;

  if (src_vma->vm_flags & (VM_HUGETLB | VM_PFNMAP | VM_MIXEDMAP)) return true;

  if (src_vma->anon_vma) return true;
...
int copy_page_range(struct vm_area_struct *dst_vma,
  ...
  if (is_cow) {
    mmu_notifier_range_init(&range, MMU_NOTIFY_PROTECTION_PAGE, 0, src_vma,
                            src_mm, addr, end);
  ...
  do {
    next = pgd_addr_end(addr, end);
    if (pgd_none_or_clear_bad(src_pgd)) continue;
    if (unlikely(
...
static inline bool should_zap_page(struct zap_details *details,
  ...
  if (should_zap_cows(details)) return true;
...
static inline void zap_install_uffd_wp_if_needed(struct vm_area_struct *vma,
  ...
  if (zap_drop_file_uffd_wp(details)) return;
...
again:
  ...
  do {
    ...
    if (pte_present(ptent)) {
      ...
      if (unlikely(!should_zap_page(details, page))) continue;
    ...
    if (is_device_private_entry(entry) || is_device_exclusive_entry(entry)) {
      ...
      rss[mm_counter(page)]--;
      if (is_device_private_entry(entry)) page_remove_rmap(page, vma, false);
      put_page(page);
    } else if (!non_swap_entry(entry)) {
      /* Genuine swap entry, hence a private anon page */
      if (!should_zap_cows(details)) continue;
      rss[MM_SWAPENTS]--;
      if (unlikely(!free_swap_and_cache(entry)))
        print_bad_pte(vma, addr, ptent, NULL);
    } else if (is_migration_entry(entry)) {
      page = pfn_swap_entry_to_page(entry);
      if (!should_zap_page(details, page)) continue;
      rss[mm_counter(page)]--;
    } else if (pte_marker_entry_uffd_wp(entry)) {
      /* Only drop the uffd-wp marker if explicitly requested */
      if (!zap_drop_file_uffd_wp(details)) continue;
    } else if (is_hwpoison_entry(entry) || is_swapin_error_entry(entry)) {
      if (!should_zap_cows(details)) continue;
    } else {
...
static inline unsigned long zap_pmd_range(struct mmu_gather *tlb,
  ...
  do {
     ...
     * mode.
     */
    if (pmd_none_or_trans_huge_or_clear_bad(pmd)) goto next;
...
static void unmap_single_vma(struct mmu_gather *tlb, struct vm_area_struct *vma,
  ...
  if (end <= vma->vm_start) return;

  if (vma->vm_file) uprobe_munmap(vma, start, end);

  if (unlikely(vma->vm_flags & VM_PFNMAP)) untrack_pfn(vma, 0, 0);
...
void zap_page_range(struct vm_area_struct *vma, unsigned long start,
  ...
  lru_add_drain();
  mmu_notifier_range_init(&range, MMU_NOTIFY_CLEAR, 0, vma, vma->vm_mm, start,
                          start + size);
...
static int validate_page_before_insert(struct page *page) {
  if (PageAnon(page) || PageSlab(page) || page_has_type(page)) return -EINVAL;
...
static int insert_page_in_batch_locked(struct vm_area_struct *vma, pte_t *pte,
  ...
  if (!page_count(page)) return -EINVAL;
...
more:
  ...
  while (pages_to_write_in_pmd) {
    ...
    for (pte = start_pte; pte_idx < batch_size; ++pte, ++pte_idx) {
      int err = insert_page_in_batch_locked(vma, pte, addr,
...
int vm_insert_pages(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (addr < vma->vm_start || end_addr >= vma->vm_end) return -EFAULT;
  if (!(vma->vm_flags & VM_MIXEDMAP)) {
...
static int __vm_map_pages(struct vm_area_struct *vma, struct page **pages,
  ...
  if (offset >= num) return -ENXIO;

  /* Fail if the user requested size exceeds available object size */
  if (count > num - offset) return -ENXIO;
...
static bool vm_mixed_ok(struct vm_area_struct *vma, pfn_t pfn) {
  ...
  if (vma->vm_flags & VM_MIXEDMAP) return true;
  if (pfn_t_devmap(pfn)) return true;
  if (pfn_t_special(pfn)) return true;
  if (is_zero_pfn(pfn_t_to_pfn(pfn))) return true;
  return false;
}
...
static vm_fault_t __vm_insert_mixed(struct vm_area_struct *vma,
  ...
  if (!pfn_modify_allowed(pfn_t_to_pfn(pfn), pgprot)) return VM_FAULT_SIGBUS;
  ...
  if (err == -ENOMEM) return VM_FAULT_OOM;
  if (err < 0 && err != -EBUSY) return VM_FAULT_SIGBUS;
...
static inline int remap_pmd_range(struct mm_struct *mm, pud_t *pud,
  ...
  do {
    next = pmd_addr_end(addr, end);
    err =
        remap_pte_range(mm, pmd, addr, next, pfn + (addr >> PAGE_SHIFT), prot);
    if (err) return err;
  } while (pmd++, addr = next, addr != end);
  return 0;
}
...
static inline int remap_pud_range(struct mm_struct *mm, p4d_t *p4d,
  ...
  do {
    next = pud_addr_end(addr, end);
    err =
        remap_pmd_range(mm, pud, addr, next, pfn + (addr >> PAGE_SHIFT), prot);
    if (err) return err;
  } while (pud++, addr = next, addr != end);
  return 0;
}
...
static inline int remap_p4d_range(struct mm_struct *mm, pgd_t *pgd,
  ...
  do {
    next = p4d_addr_end(addr, end);
    err =
        remap_pud_range(mm, p4d, addr, next, pfn + (addr >> PAGE_SHIFT), prot);
    if (err) return err;
  } while (p4d++, addr = next, addr != end);
  return 0;
}
...
int remap_pfn_range_notrack(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (is_cow_mapping(vma->vm_flags)) {
    if (addr != vma->vm_start || end != vma->vm_end) return -EINVAL;
    vma->vm_pgoff = pfn;
  }
...
int remap_pfn_range(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (err) untrack_pfn(vma, pfn, PAGE_ALIGN(size));
  return err;
}
...
int vm_iomap_memory(struct vm_area_struct *vma, phys_addr_t start,
  ...
  if (start + len < start) return -EINVAL;
  /*
  ...
  if (pfn + pages < pfn) return -EINVAL;

  /* We start the mapping 'vm_pgoff' pages into the area */
  if (vma->vm_pgoff > pages) return -EINVAL;
  pfn += vma->vm_pgoff;
  pages -= vma->vm_pgoff;

  /* Can we fit all of the mapping? */
  vm_len = vma->vm_end - vma->vm_start;
  if (vm_len >> PAGE_SHIFT > pages) return -EINVAL;
...
static int apply_to_pmd_range(struct mm_struct *mm, pud_t *pud,
  ...
  do {
    next = pmd_addr_end(addr, end);
    if (pmd_none(*pmd) && !create) continue;
    if (WARN_ON_ONCE(pmd_leaf(*pmd))) return -EINVAL;
...
static int apply_to_pud_range(struct mm_struct *mm, p4d_t *p4d,
  ...
  do {
    next = pud_addr_end(addr, end);
    if (pud_none(*pud) && !create) continue;
    if (WARN_ON_ONCE(pud_leaf(*pud))) return -EINVAL;
...
static int apply_to_p4d_range(struct mm_struct *mm, pgd_t *pgd,
  ...
  do {
    next = p4d_addr_end(addr, end);
    if (p4d_none(*p4d) && !create) continue;
    if (WARN_ON_ONCE(p4d_leaf(*p4d))) return -EINVAL;
...
static int __apply_to_page_range(struct mm_struct *mm, unsigned long addr,
  ...
  do {
    next = pgd_addr_end(addr, end);
    if (pgd_none(*pgd) && !create) continue;
    if (WARN_ON_ONCE(pgd_leaf(*pgd))) return -EINVAL;
...
pte_unlock:
  if (locked) pte_unmap_unlock(vmf->pte, vmf->ptl);
  kunmap_atomic(kaddr);
...
static vm_fault_t fault_dirty_shared_page(struct vm_fault *vmf) {
  ...
  if (!page_mkwrite) file_update_time(vma->vm_file);
...
static vm_fault_t wp_page_copy(struct vm_fault *vmf) {
  ...
  if (likely(pte_same(*vmf->pte, vmf->orig_pte))) {
    ...
    if (unlikely(unshare)) {
      if (pte_soft_dirty(vmf->orig_pte)) entry = pte_mksoft_dirty(entry);
...
static vm_fault_t wp_pfn_shared(struct vm_fault *vmf) {
  ...
  if (vma->vm_ops && vma->vm_ops->pfn_mkwrite) {
    ...
    if (ret & (VM_FAULT_ERROR | VM_FAULT_NOPAGE)) return ret;
...
static vm_fault_t do_wp_page(struct vm_fault *vmf) __releases(vmf->ptl) {
  ...
  if (PageAnon(vmf->page)) {
     ...
     */
    if (PageKsm(page) || page_count(page) > 3) goto copy;
    if (!PageLRU(page))
      /*
       ...
       */
      lru_add_drain();
    if (page_count(page) > 1 + PageSwapCache(page)) goto copy;
...
copy:
  ...
  pte_unmap_unlock(vmf->pte, vmf->ptl);
#ifdef CONFIG_KSM
  if (PageKsm(vmf->page)) count_vm_event(COW_KSM);
#endif
  return wp_page_copy(vmf);
}
...
void unmap_mapping_folio(struct folio *folio) {
  ...
  if (unlikely(!RB_EMPTY_ROOT(&mapping->i_mmap.rb_root)))
    unmap_mapping_range_tree(&mapping->i_mmap, first_index, last_index,
                             &details);
...
void unmap_mapping_pages(struct address_space *mapping, pgoff_t start,
  ...
  if (last_index < first_index) last_index = ULONG_MAX;
...
void unmap_mapping_range(struct address_space *mapping, loff_t const holebegin,
  ...
  if (sizeof(holelen) > sizeof(hlen)) {
    ...
    if (holeend & ~(long long)ULONG_MAX) hlen = ULONG_MAX - hba + 1;
  }
...
static vm_fault_t remove_device_exclusive_entry(struct vm_fault *vmf) {
  ...
  if (!lock_page_or_retry(page, vma->vm_mm, vmf->flags)) return VM_FAULT_RETRY;
...
static inline bool should_try_to_free_swap(struct page *page,
  ...
  if (!PageSwapCache(page)) return false;
...
vm_fault_t do_swap_page(struct vm_fault *vmf) {
  ...
  if (!pte_unmap_same(vmf)) goto out;
  ...
  if (!page) {
    if (data_race(si->flags & SWP_SYNCHRONOUS_IO) && __swap_count(entry) == 1) {
      ...
      if (page) {
        ...
        if (mem_cgroup_swapin_charge_page(page, vma->vm_mm, GFP_KERNEL,
                                          entry)) {
          ret = VM_FAULT_OOM;
          goto out_page;
        }
        ...
        if (shadow) workingset_refault(page_folio(page), shadow);
    ...
    if (!page) {
       ...
       */
      vmf->pte =
          pte_offset_map_lock(vma->vm_mm, vmf->pmd, vmf->address, &vmf->ptl);
      if (likely(pte_same(*vmf->pte, vmf->orig_pte))) ret = VM_FAULT_OOM;
      goto unlock;
    }
   ...
   */
  swap_free(entry);
  if (should_try_to_free_swap(page, vma, vmf->flags)) try_to_free_swap(page);
  ...
  if (pte_swp_soft_dirty(vmf->orig_pte)) pte = pte_mksoft_dirty(pte);
...
static vm_fault_t __do_fault(struct vm_fault *vmf) {
  ...
  if (pmd_none(*vmf->pmd) && !vmf->prealloc_pte) {
    ...
    if (!vmf->prealloc_pte) return VM_FAULT_OOM;
  }
...
vm_fault_t do_set_pmd(struct vm_fault *vmf, struct page *page) {
  ...
  if (compound_order(page) != HPAGE_PMD_ORDER) return ret;
   ...
   */
  if (unlikely(PageHasHWPoisoned(page))) return ret;
  ...
  if (unlikely(!pmd_none(*vmf->pmd))) goto out;

  for (i = 0; i < HPAGE_PMD_NR; i++) flush_icache_page(vma, page + i);

  entry = mk_huge_pmd(page, vma->vm_page_prot);
  if (write) entry = maybe_pmd_mkwrite(pmd_mkdirty(entry), vma);
   ...
   */
  if (arch_needs_pgtable_deposit()) deposit_prealloc_pte(vmf);
...
void do_set_pte(struct vm_fault *vmf, struct page *page, unsigned long addr) {
  ...
  if (unlikely(uffd_wp)) entry = pte_mkuffd_wp(pte_wrprotect(entry));
  /* copy-on-write page */
...
vm_fault_t finish_fault(struct vm_fault *vmf) {
   ...
   */
  if (pmd_devmap_trans_unstable(vmf->pmd)) return VM_FAULT_NOPAGE;
...
static inline bool should_fault_around(struct vm_fault *vmf) {
  ...
  if (!vmf->vma->vm_ops->map_pages) return false;

  if (uffd_disable_fault_around(vmf->vma)) return false;
...
static vm_fault_t do_cow_fault(struct vm_fault *vmf) {
  ...
  if (mem_cgroup_charge(page_folio(vmf->cow_page), vma->vm_mm, GFP_KERNEL)) {
  ...
  if (unlikely(ret & (VM_FAULT_ERROR | VM_FAULT_NOPAGE | VM_FAULT_RETRY)))
    goto uncharge_out;
  if (ret & VM_FAULT_DONE_COW) return ret;
...
static vm_fault_t do_numa_page(struct vm_fault *vmf) {
  ...
  if (!page) goto out_map;

  /* TODO: handle PTE-mapped THP */
  if (PageCompound(page)) goto out_map;
   ...
   */
  if (!was_writable) flags |= TNF_NO_GROUP;
...
static inline vm_fault_t create_huge_pmd(struct vm_fault *vmf) {
  if (vma_is_anonymous(vmf->vma)) return do_huge_pmd_anonymous_page(vmf);
...
static inline vm_fault_t wp_huge_pmd(struct vm_fault *vmf) {
  ...
  if (vma_is_anonymous(vmf->vma)) {
    if (likely(!unshare) && userfaultfd_huge_pmd_wp(vmf->vma, vmf->orig_pmd))
      return handle_userfault(vmf, VM_UFFD_WP);
...
static vm_fault_t wp_huge_pud(struct vm_fault *vmf, pud_t orig_pud) {
    ...
    defined(CONFIG_HAVE_ARCH_TRANSPARENT_HUGEPAGE_PUD)
  /* No support for anonymous transparent PUD pages yet */
  if (vma_is_anonymous(vmf->vma)) goto split;
  if (vmf->vma->vm_ops->huge_fault) {
...
static vm_fault_t handle_pte_fault(struct vm_fault *vmf) {
  ...
  if (ptep_set_access_flags(vmf->vma, vmf->address, vmf->pte, entry,
    ...
    if (vmf->flags & FAULT_FLAG_TRIED) goto unlock;
    /*
...
retry_pud:
  ...
  if (pud_trans_unstable(vmf.pud)) goto retry_pud;
...
static inline void mm_account_fault(struct pt_regs *regs, unsigned long address,
   ...
   */
  if (ret & (VM_FAULT_ERROR | VM_FAULT_RETRY)) return;
...
int follow_pte(struct mm_struct *mm, unsigned long address, pte_t **ptepp,
  ...
  if (p4d_none(*p4d) || unlikely(p4d_bad(*p4d))) goto out;
  ...
  if (!pte_present(*ptep)) goto unlock;
  *ptepp = ptep;
  return 0;
unlock:
...
int follow_pfn(struct vm_area_struct *vma, unsigned long address,
  ...
  if (!(vma->vm_flags & (VM_IO | VM_PFNMAP))) return ret;
...
int follow_phys(struct vm_area_struct *vma, unsigned long address,
  ...
  if (follow_pte(vma->vm_mm, address, &ptep, &ptl)) goto out;
  pte = *ptep;
...
int generic_access_phys(struct vm_area_struct *vma, unsigned long addr,
  ...
  if (!(vma->vm_flags & (VM_IO | VM_PFNMAP))) return -EINVAL;

retry:
  if (follow_pte(vma->vm_mm, addr, &ptep, &ptl)) return -EINVAL;
  pte = *ptep;
  pte_unmap_unlock(ptep, ptl);
  ...
  if (!maddr) return -ENOMEM;

  if (follow_pte(vma->vm_mm, addr, &ptep, &ptl)) goto out_unmap;
...
void print_vma_addr(char *prefix, unsigned long ip) {
  ...
  if (vma && vma->vm_file) {
    ...
    if (buf) {
      ...
      if (IS_ERR(p)) p = "?";
      printk("%s%s[%lx+%lx]", prefix, kbasename(p), vma->vm_start,
...
void __might_fault(const char *file, int line) {
  ...
  if (current->mm) might_lock_read(&current->mm->mmap_lock);
#endif
}
...
static inline void process_huge_page(
  ...
  int i, n, base, l;
  unsigned long addr =
      addr_hint & ~(((unsigned long)pages_per_huge_page << PAGE_SHIFT) - 1);
...
long copy_huge_page_from_user(struct page *dst_page, const void __user *usr_src,
  ...
  for (i = 0; i < pages_per_huge_page;
    ...
    rc = copy_from_user(page_kaddr, usr_src + i * PAGE_SIZE, PAGE_SIZE);
    if (allow_pagefault)
      kunmap(subpage);
    else
...
void __init ptlock_cache_init(void) {
  page_ptl_cachep =
      kmem_cache_create("page->ptl", sizeof(spinlock_t), 0, SLAB_PANIC, NULL);
}
