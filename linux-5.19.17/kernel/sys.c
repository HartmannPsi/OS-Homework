...
SYSCALL_DEFINE3(setpriority, int, which, int, who, int, niceval) {
  ...
  error = -ESRCH;
  if (niceval < MIN_NICE) niceval = MIN_NICE;
  if (niceval > MAX_NICE) niceval = MAX_NICE;
  ...
  switch (which) {
    case PRIO_PROCESS:
      if (who)
        ...
        p = current;
      if (p) error = set_one_prio(p, niceval, error);
      break;
    case PRIO_PGRP:
      if (who)
        pgrp = find_vpid(who);
    ...
    case PRIO_USER:
      ...
      if (!who)
        ...
        user = find_user(uid);
        if (!user) goto out_unlock; /* No processes for this user */
      }
      for_each_process_thread(g, p) {
        if (uid_eq(task_uid(p), uid) && task_pid_vnr(p))
          error = set_one_prio(p, niceval, error);
      }
      if (!uid_eq(uid, cred->uid)) free_uid(user); /* For find_user() */
      break;
  }
out_unlock:
  rcu_read_unlock();
...
long __sys_setregid(gid_t rgid, gid_t egid) {
  ...
  if ((rgid != (gid_t)-1) && !gid_valid(krgid)) return -EINVAL;
  if ((egid != (gid_t)-1) && !gid_valid(kegid)) return -EINVAL;
  ...
  if (egid != (gid_t)-1) {
    if (gid_eq(old->gid, kegid) || gid_eq(old->egid, kegid) ||
        gid_eq(old->sgid, kegid) || ns_capable_setid(old->user_ns, CAP_SETGID))
      new->egid = kegid;
    else
      goto error;
  }
...
long __sys_setreuid(uid_t ruid, uid_t euid) {
  ...
  if ((ruid != (uid_t)-1) && !uid_valid(kruid)) return -EINVAL;
  if ((euid != (uid_t)-1) && !uid_valid(keuid)) return -EINVAL;
...
long __sys_setresuid(uid_t ruid, uid_t euid, uid_t suid) {
  ...
  if ((suid != (uid_t)-1) && !uid_valid(ksuid)) return -EINVAL;
  ...
  if (euid != (uid_t)-1) new->euid = keuid;
  if (suid != (uid_t)-1) new->suid = ksuid;
  new->fsuid = new->euid;
...
SYSCALL_DEFINE3(getresuid, uid_t __user *, ruidp, uid_t __user *, euidp,
  ...
  if (!retval) {
    ...
    if (!retval) return put_user(suid, suidp);
  }
  return retval;
}
...
long __sys_setresgid(gid_t rgid, gid_t egid, gid_t sgid) {
  ...
  if ((rgid != (gid_t)-1) && !gid_valid(krgid)) return -EINVAL;
  if ((egid != (gid_t)-1) && !gid_valid(kegid)) return -EINVAL;
  if ((sgid != (gid_t)-1) && !gid_valid(ksgid)) return -EINVAL;
...
SYSCALL_DEFINE3(getresgid, gid_t __user *, rgidp, gid_t __user *, egidp,
  ...
  if (!retval) {
    ...
    if (!retval) retval = put_user(sgid, sgidp);
  }
...
long __sys_setfsuid(uid_t uid) {
  ...
  if (!uid_valid(kuid)) return old_fsuid;

  new = prepare_creds();
  if (!new) return old_fsuid;
...
long __sys_setfsgid(gid_t gid) {
  ...
  if (!gid_valid(kgid)) return old_fsgid;

  new = prepare_creds();
  if (!new) return old_fsgid;
...
COMPAT_SYSCALL_DEFINE1(times, struct compat_tms __user *, tbuf) {
  if (tbuf) {
    ...
    if (copy_to_user(tbuf, &tmp, sizeof(tmp))) return -EFAULT;
  }
  force_successful_syscall_return();
...
SYSCALL_DEFINE2(setpgid, pid_t, pid, pid_t, pgid) {
  ...
  if (task_pgrp(p) != pgrp) change_pid(p, PIDTYPE_PGID, pgrp);
...
SYSCALL_DEFINE0(getpgrp) { return do_getpgid(0); }
...
static void set_special_pids(struct pid *pid) {
  ...
  if (task_session(curr) != pid) change_pid(curr, PIDTYPE_SID, pid);

  if (task_pgrp(curr) != pid) change_pid(curr, PIDTYPE_PGID, pid);
}
...
int ksys_setsid(void) {
  ...
  if (group_leader->signal->leader) goto out;
...
SYSCALL_DEFINE1(newuname, struct new_utsname __user *, name) {
  ...
  if (override_release(name->release, sizeof(name->release))) return -EFAULT;
  if (override_architecture(name)) return -EFAULT;
  return 0;
}
...
SYSCALL_DEFINE2(gethostname, char __user *, name, int, len) {
  ...
  up_read(&uts_sem);
  if (copy_to_user(name, tmp, i)) return -EFAULT;
  return 0;
}
...
static int do_prlimit(struct task_struct *tsk, unsigned int resource,
  ...
  if (new_rlim) {
    ...
    if (new_rlim->rlim_max > rlim->rlim_max && !capable(CAP_SYS_RESOURCE))
      retval = -EPERM;
    if (!retval) retval = security_task_setrlimit(tsk, resource, new_rlim);
  }
  if (!retval) {
...
SYSCALL_DEFINE2(getrlimit, unsigned int, resource, struct rlimit __user *,
  ...
  if (!ret) ret = copy_to_user(rlim, &value, sizeof(*rlim)) ? -EFAULT : 0;
...
COMPAT_SYSCALL_DEFINE2(setrlimit, unsigned int, resource,
  ...
  if (copy_from_user(&r32, rlim, sizeof(struct compat_rlimit))) return -EFAULT;
...
COMPAT_SYSCALL_DEFINE2(getrlimit, unsigned int, resource,
  ...
  if (!ret) {
    ...
    if (copy_to_user(rlim, &r32, sizeof(struct compat_rlimit))) return -EFAULT;
  }
  return ret;
}
...
SYSCALL_DEFINE2(old_getrlimit, unsigned int, resource, struct rlimit __user *,
  ...
  if (x.rlim_cur > 0x7FFFFFFF) x.rlim_cur = 0x7FFFFFFF;
  if (x.rlim_max > 0x7FFFFFFF) x.rlim_max = 0x7FFFFFFF;
...
COMPAT_SYSCALL_DEFINE2(old_getrlimit, unsigned int, resource,
  ...
  task_unlock(current->group_leader);
  if (r.rlim_cur > 0x7FFFFFFF) r.rlim_cur = 0x7FFFFFFF;
  if (r.rlim_max > 0x7FFFFFFF) r.rlim_max = 0x7FFFFFFF;
...
static int check_prlimit_permission(struct task_struct *task,
  ...
  id_match =
      (uid_eq(cred->uid, tcred->euid) && uid_eq(cred->uid, tcred->suid) &&
       uid_eq(cred->uid, tcred->uid) && gid_eq(cred->gid, tcred->egid) &&
       gid_eq(cred->gid, tcred->sgid) && gid_eq(cred->gid, tcred->gid));
  if (!id_match && !ns_capable(tcred->user_ns, CAP_SYS_RESOURCE)) return -EPERM;
...
SYSCALL_DEFINE4(prlimit64, pid_t, pid, unsigned int, resource,
  ...
  if (new_rlim) {
    if (copy_from_user(&new64, new_rlim, sizeof(new64))) return -EFAULT;
  ...
  ret =
      do_prlimit(tsk, resource, new_rlim ? &new : NULL, old_rlim ? &old : NULL);

  if (!ret && old_rlim) {
    rlim_to_rlim64(&old, &old64);
    if (copy_to_user(old_rlim, &old64, sizeof(old64))) ret = -EFAULT;
  }
...
void getrusage(struct task_struct *p, int who, struct rusage *r) {
  ...
  if (!lock_task_sighand(p, &flags)) return;
...
static int prctl_set_mm_exe_file(struct mm_struct *mm, unsigned int fd) {
   ...
   */
  err = -EACCES;
  if (!S_ISREG(inode->i_mode) || path_noexec(&exe.file->f_path)) goto exit;
...
static int prctl_set_mm_map(int opt, const void __user *addr,
  ...
  if (prctl_map.auxv_size) {
    ...
    if (copy_from_user(user_auxv, (const void __user *)prctl_map.auxv,
                       prctl_map.auxv_size))
      return -EFAULT;
   ...
   * more complex.
   */
  if (prctl_map.auxv_size) memcpy(mm->saved_auxv, user_auxv, sizeof(user_auxv));
...
static int prctl_set_auxv(struct mm_struct *mm, unsigned long addr,
  ...
  if (copy_from_user(user_auxv, (const void __user *)addr, len)) return -EFAULT;
...
static int propagate_has_child_subreaper(struct task_struct *p, void *data) {
   ...
   */
  if (p->signal->has_child_subreaper || is_child_reaper(task_pid(p))) return 0;
...
static inline bool is_valid_name_char(char ch) {
  ...
  return ch > 0x1f && ch < 0x7f && !strchr(ANON_VMA_NAME_INVALID_CHARS, ch);
}
...
SYSCALL_DEFINE5(prctl, int, option, unsigned long, arg2, unsigned long, arg3,
  ...
  switch (option) {
    ...
    case PR_GET_TIMING:
      ...
      break;
    case PR_SET_TIMING:
      if (arg2 != PR_TIMING_STATISTICAL) error = -EINVAL;
      break;
    case PR_SET_NAME:
      comm[sizeof(me->comm) - 1] = 0;
      if (strncpy_from_user(comm, (char __user *)arg2, sizeof(me->comm) - 1) <
          0)
        return -EFAULT;
    ...
    case PR_GET_NAME:
      ...
      if (copy_to_user((char __user *)arg2, comm, sizeof(comm))) return -EFAULT;
      break;
    case PR_GET_ENDIAN:
    ...
    case PR_MCE_KILL_GET:
      ...
      if (current->flags & PF_MCE_PROCESS)
        error = (current->flags & PF_MCE_EARLY) ? PR_MCE_KILL_EARLY
                                                : PR_MCE_KILL_LATE;
      else
    ...
    case PR_SET_THP_DISABLE:
      ...
      if (mmap_write_lock_killable(me->mm)) return -EINTR;
      if (arg2)
...
static int do_sysinfo(struct sysinfo *info) {
  ...
  if (mem_total < info->totalram || mem_total < info->totalswap) goto out;
  bitcount = 0;
  ...
  while (mem_unit > 1) {
    ...
    sav_total = mem_total;
    mem_total <<= 1;
    if (mem_total < sav_total) goto out;
  }
...
SYSCALL_DEFINE1(sysinfo, struct sysinfo __user *, info) {
  ...
  if (copy_to_user(info, &val, sizeof(struct sysinfo))) return -EFAULT;
...
COMPAT_SYSCALL_DEFINE1(sysinfo, struct compat_sysinfo __user *, info) {
  ...
  if (copy_to_user(info, &s_32, sizeof(s_32))) return -EFAULT;
  return 0;
}
#endif /* CONFIG_COMPAT */
