...
static inline bool __sched_core_less(struct task_struct *a,
  ...
  if (a->core_cookie < b->core_cookie) return true;

  if (a->core_cookie > b->core_cookie) return false;

  /* flip prio, so high prio is leftmost */
  if (prio_less(b, a, !!task_rq(a)->core->core_forceidle_count)) return true;
...
static inline int rb_sched_core_cmp(const void *key,
                                    const struct rb_node *node) {
  const struct task_struct *p = __node_2_sc(node);
  unsigned long cookie = (unsigned long)key;

  if (cookie < p->core_cookie) return -1;
...
static struct task_struct *sched_core_find(struct rq *rq,
   ...
   */
  if (!node) return idle_sched_class.pick_task(rq);
...
static struct task_struct *sched_core_next(struct task_struct *p,
  ...
  if (p->core_cookie != cookie) return NULL;
...
static void sched_core_lock(int cpu, unsigned long *flags) {
  ...
  for_each_cpu(t, smt_mask) raw_spin_lock_nested(&cpu_rq(t)->__lock, i++);
}
...
static void sched_core_unlock(int cpu, unsigned long *flags) {
  ...
  for_each_cpu(t, smt_mask) raw_spin_unlock(&cpu_rq(t)->__lock);
...
static void __sched_core_flip(bool enabled) {
  ...
  for_each_cpu(cpu, &sched_core_mask) cpu_rq(cpu)->core_enabled = enabled;
...
void sched_core_get(void) {
  if (atomic_inc_not_zero(&sched_core_count)) return;

  mutex_lock(&sched_core_mutex);
  if (!atomic_read(&sched_core_count)) __sched_core_enable();
...
void sched_core_put(void) {
   ...
   */
  if (!atomic_add_unless(&sched_core_count, -1, 1)) schedule_work(&_work);
}
...
void raw_spin_rq_unlock(struct rq *rq) { raw_spin_unlock(rq_lockp(rq)); }
...
struct rq *__task_rq_lock(struct task_struct *p, struct rq_flags *rf)
  ...
  for (;;) {
    ...
    while (unlikely(task_on_rq_migrating(p))) cpu_relax();
  }
}
...
static bool __wake_q_add(struct wake_q_head *head, struct task_struct *task) {
   ...
   */
  smp_mb__before_atomic();
  if (unlikely(cmpxchg_relaxed(&node->next, NULL, WAKE_Q_TAIL))) return false;
...
void wake_q_add(struct wake_q_head *head, struct task_struct *task) {
  if (__wake_q_add(head, task)) get_task_struct(task);
}
...
void wake_q_add_safe(struct wake_q_head *head, struct task_struct *task) {
  if (!__wake_q_add(head, task)) put_task_struct(task);
}
...
void resched_cpu(int cpu) {
  ...
  if (cpu_online(cpu) || cpu == smp_processor_id()) resched_curr(rq);
...
int get_nohz_timer_target(void) {
  ...
  if (default_cpu == -1) default_cpu = housekeeping_any_cpu(HK_TYPE_TIMER);
  cpu = default_cpu;
unlock:
  rcu_read_unlock();
  return cpu;
...
static bool wake_up_full_nohz_cpu(int cpu) {
   ...
   * empty IRQ.
   */
  if (cpu_is_offline(cpu)) return true; /* Don't try to wake offline CPUs. */
...
static void nohz_csd_func(void *info) {
   ...
   */
  flags =
      atomic_fetch_andnot(NOHZ_KICK_MASK | NOHZ_NEWILB_KICK, nohz_flags(cpu));
...
bool sched_can_stop_tick(struct rq *rq) {
  ...
  if (rq->dl.dl_nr_running) return false;
  ...
  if (fifo_nr_running) return true;
...
static inline unsigned int uclamp_none(enum uclamp_id clamp_id) {
  if (clamp_id == UCLAMP_MIN) return 0;
...
static inline void uclamp_idle_reset(struct rq *rq, enum uclamp_id clamp_id,
  ...
  if (!(rq->uclamp_flags & UCLAMP_FLAG_IDLE)) return;
...
static inline unsigned int uclamp_rq_max_value(struct rq *rq,
  ...
  for (; bucket_id >= 0; bucket_id--) {
    if (!bucket[bucket_id].tasks) continue;
...
static inline struct uclamp_se uclamp_tg_restrict(struct task_struct *p,
   ...
   */
  if (task_group_is_autogroup(task_group(p))) return uc_req;
  if (task_group(p) == &root_task_group) return uc_req;
...
static inline struct uclamp_se uclamp_eff_get(struct task_struct *p,
  ...
  if (unlikely(uc_req.value > uc_max.value)) return uc_max;
...
static inline void uclamp_rq_dec_id(struct rq *rq, struct task_struct *p,
  ...
  if (likely(bucket->tasks)) bucket->tasks--;
...
static inline void uclamp_rq_inc(struct rq *rq, struct task_struct *p) {
   ...
   */
  if (!static_branch_unlikely(&sched_uclamp_used)) return;
...
static inline void uclamp_rq_reinc_id(struct rq *rq, struct task_struct *p,
                                      enum uclamp_id clamp_id) {
  if (!p->uclamp[clamp_id].active) return;
...
static inline void uclamp_update_active_tasks(struct cgroup_subsys_state *css) {
  ...
  while ((p = css_task_iter_next(&it))) uclamp_update_active(p);
...
static int uclamp_validate(struct task_struct *p,
  ...
  if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MIN) {
    ...
    if (util_min + 1 > SCHED_CAPACITY_SCALE + 1) return -EINVAL;
  }

  if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MAX) {
    util_max = attr->sched_util_max;

    if (util_max + 1 > SCHED_CAPACITY_SCALE + 1) return -EINVAL;
  }
...
static bool uclamp_reset(const struct sched_attr *attr, enum uclamp_id clamp_id,
  ...
  if (clamp_id == UCLAMP_MIN && attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MIN &&
      attr->sched_util_min == -1) {
    return true;
  }

  if (clamp_id == UCLAMP_MAX && attr->sched_flags & SCHED_FLAG_UTIL_CLAMP_MAX &&
...
static void __setscheduler_uclamp(struct task_struct *p,
  ...
  for_each_clamp_id(clamp_id) {
    ...
    if (!uclamp_reset(attr, clamp_id, uc_se)) continue;
  ...
  if (likely(!(attr->sched_flags & SCHED_FLAG_UTIL_CLAMP))) return;
...
static void uclamp_fork(struct task_struct *p) {
  ...
  if (likely(!p->sched_reset_on_fork)) return;
...
bool sched_task_on_rq(struct task_struct *p) { return task_on_rq_queued(p); }
...
static inline void dequeue_task(struct rq *rq, struct task_struct *p,
                                int flags) {
  if (sched_core_enabled(rq)) sched_core_dequeue(rq, p, flags);

  if (!(flags & DEQUEUE_NOCLOCK)) update_rq_clock(rq);
...
void migrate_enable(void) {
  ...
  if (WARN_ON_ONCE(!p->migration_disabled)) return;
...
static inline bool rq_has_pinned_tasks(struct rq *rq) { return rq->nr_pinned; }
...
static inline bool is_cpu_allowed(struct task_struct *p, int cpu) {
  ...
  if (!cpumask_test_cpu(cpu, p->cpus_ptr)) return false;

  /* migrate_disabled() must be allowed to finish. */
  if (is_migration_disabled(p)) return cpu_online(cpu);
  ...
  if (kthread_is_per_cpu(p)) return cpu_online(cpu);

  /* Regular kernel threads don't get to stay during offline. */
  if (cpu_dying(cpu)) return false;
...
static struct rq *__migrate_task(struct rq *rq, struct rq_flags *rf,
  ...
  if (!is_cpu_allowed(p, dest_cpu)) return rq;
...
static int migration_cpu_stop(void *data) {
  ...
  if (task_rq(p) == rq) {
    ...
    task_rq_unlock(rq, p, &rf);
    stop_one_cpu_nowait(task_cpu(p), migration_cpu_stop, &pending->arg,
                        &pending->stop_work);
    return 0;
  }
out:
...
int push_cpu_stop(void *arg) {
  ...
  if (task_rq(p) != rq) goto out_unlock;
...
int dup_user_cpus_ptr(struct task_struct *dst, struct task_struct *src,
  ...
  if (!dst->user_cpus_ptr) return -ENOMEM;
...
static int affine_move_task(struct rq *rq, struct task_struct *p,
  ...
  if (cpumask_test_cpu(task_cpu(p), &p->cpus_mask)) {
    ...
    if ((flags & SCA_MIGRATE_ENABLE) && (p->migration_flags & MDF_PUSH) &&
        !rq->push_busy) {
      rq->push_busy = true;
  ...
  if (task_running(rq, p) || READ_ONCE(p->__state) == TASK_WAKING) {
    ...
    if (!stop_pending) {
      stop_one_cpu_nowait(cpu_of(rq), migration_cpu_stop, &pending->arg,
                          &pending->stop_work);
    }
    ...
    if (!is_migration_disabled(p)) {
      if (task_on_rq_queued(p)) rq = move_queued_task(rq, rf, p, dest_cpu);
...
static int __set_cpus_allowed_ptr_locked(struct task_struct *p,
  ...
  if (!(flags & SCA_MIGRATE_ENABLE)) {
    if (cpumask_equal(&p->cpus_mask, new_mask)) goto out;
  ...
  if (flags & SCA_USER) user_mask = clear_user_cpus_ptr(p);
...
void force_compatible_cpus_allowed_ptr(struct task_struct *p) {
   ...
   */
  cpus_read_lock();
  if (!cpumask_available(new_mask)) goto out_set_mask;
...
void relax_compatible_cpus_allowed_ptr(struct task_struct *p) {
   ...
   * a subsequent fork().
   */
  if (!user_mask || !__sched_setaffinity(p, user_mask)) return;
...
void set_task_cpu(struct task_struct *p, unsigned int new_cpu) {
   ...
   */
  WARN_ON_ONCE(state == TASK_RUNNING && p->sched_class == &fair_sched_class &&
...
static int migrate_swap_stop(void *data) {
  ...
  if (!cpumask_test_cpu(arg->dst_cpu, arg->src_task->cpus_ptr)) goto unlock;

  if (!cpumask_test_cpu(arg->src_cpu, arg->dst_task->cpus_ptr)) goto unlock;
...
static int select_fallback_rq(int cpu, struct task_struct *p) {
  ...
  if (nid != -1) {
    ...
    for_each_cpu(dest_cpu, nodemask) {
      if (is_cpu_allowed(p, dest_cpu)) return dest_cpu;
    }
  }
  ...
  for (;;) {
    ...
    for_each_cpu(dest_cpu, p->cpus_ptr) {
      if (!is_cpu_allowed(p, dest_cpu)) continue;
 ...
 */
static inline int select_task_rq(struct task_struct *p, int cpu,
                                 int wake_flags) {
  lockdep_assert_held(&p->pi_lock);
...
static inline void migrate_disable_switch(struct rq *rq,
                                          struct task_struct *p) {}

static inline bool rq_has_pinned_tasks(struct rq *rq) { return false; }
...
static void ttwu_stat(struct task_struct *p, int cpu, int wake_flags) {
  ...
  if (wake_flags & WF_MIGRATED) __schedstat_inc(p->stats.nr_wakeups_migrate);
#endif /* CONFIG_SMP */

  __schedstat_inc(rq->ttwu_count);
  __schedstat_inc(p->stats.nr_wakeups);

  if (wake_flags & WF_SYNC) __schedstat_inc(p->stats.nr_wakeups_sync);
}
...
void sched_ttwu_pending(void *arg) {
  ...
  llist_for_each_entry_safe(p, t, llist, wake_entry.llist) {
    if (WARN_ON_ONCE(p->on_cpu)) smp_cond_load_acquire(&p->on_cpu, !VAL);

    if (WARN_ON_ONCE(task_cpu(p) != cpu_of(rq))) set_task_cpu(p, cpu_of(rq));
...
void wake_up_if_idle(int cpu) {
  ...
  rq_lock_irqsave(rq, &rf);
  if (is_idle_task(rq->curr)) resched_curr(rq);
...
static inline bool ttwu_queue_cond(struct task_struct *p, int cpu) {
   ...
   * in hotplug state.
   */
  if (!cpu_active(cpu)) return false;
   ...
   */
  if (!cpus_share_cache(smp_processor_id(), cpu)) return true;

  if (cpu == smp_processor_id()) return false;
   ...
   */
  if (!cpu_rq(cpu)->nr_running) return true;
...
static __always_inline bool ttwu_state_match(struct task_struct *p,
  ...
  if (IS_ENABLED(CONFIG_DEBUG_PREEMPT)) {
    WARN_ON_ONCE((state & TASK_RTLOCK_WAIT) && state != TASK_RTLOCK_WAIT);
  }
...
static int try_to_wake_up(struct task_struct *p, unsigned int state,
  ...
  raw_spin_lock_irqsave(&p->pi_lock, flags);
  smp_mb__after_spinlock();
  if (!ttwu_state_match(p, state, &success)) goto unlock;
   ...
   * __schedule() (switch to task 'p')
   *   LOCK rq->lock
   *   smp_mb__after_spin_lock() smp_cond_load_acquire(&p->on_cpu) STORE
   * p->on_cpu = 1		LOAD p->cpu
   *
...
unlock:
  raw_spin_unlock_irqrestore(&p->pi_lock, flags);
out:
  if (success) ttwu_stat(p, task_cpu(p), wake_flags);
  preempt_enable();
...
void force_schedstat_enabled(void) {
  if (!schedstat_enabled()) {
    pr_info(
        "kernel profiling enabled schedstats, disable via "
...
out:
  if (!ret) pr_warn("Unable to parse schedstats=\n");
...
unsigned long to_ratio(u64 period, u64 runtime) {
  if (runtime == RUNTIME_INF) return BW_UNIT;
...
static inline void kmap_local_sched_out(void) {
  ...
  if (unlikely(current->kmap_ctrl.idx)) __kmap_local_sched_out();
#endif
}
...
static u64 cpu_resched_latency(struct rq *rq) {
  ...
  if (resched_latency <= latency_warn_ms * NSEC_PER_MSEC) return 0;
...
void scheduler_tick(void) {
  ...
  if (sched_feat(LATENCY_WARN)) resched_latency = cpu_resched_latency(rq);
...
static void sched_tick_remote(struct work_struct *work) {
   ...
   */
  if (!tick_nohz_tick_stopped_cpu(cpu)) goto out_requeue;

  rq_lock_irq(rq, &rf);
  curr = rq->curr;
  if (cpu_is_offline(cpu)) goto out_unlock;
...
static void sched_tick_start(int cpu) {
  ...
  if (housekeeping_cpu(cpu, HK_TYPE_TICK)) return;
...
static void put_prev_task_balance(struct rq *rq, struct task_struct *prev,
  ...
  for_class_range(class, prev->sched_class, &idle_sched_class) {
    if (class->balance(rq, prev, rf)) break;
  }
#endif
...
static inline bool cookie_match(struct task_struct *a, struct task_struct *b) {
  if (is_task_rq_idle(a) || is_task_rq_idle(b)) return true;
...
static struct task_struct *pick_next_task(struct rq *rq,
  ...
  for_each_cpu_wrap(i, smt_mask, cpu) {
    ...
    if (!max || prio_less(max, p, fi_before)) max = p;
  }
  ...
  for_each_cpu(i, smt_mask) {
    ...
    if (!cookie_equals(p, cookie)) {
      ...
      if (!p) p = idle_sched_class.pick_task(rq_i);
    }
    ...
    if (p == rq_i->idle) {
      if (rq_i->nr_running) {
        ...
        if (!fi_before) rq->core->core_forceidle_seq++;
      }
    } else {
      occ++;
    }
...
static bool try_steal_cookie(int this, int that) {
  ...
  if (!cookie) goto unlock;

  if (dst->curr != dst->idle) goto unlock;

  p = sched_core_find(src, cookie);
  if (p == src->idle) goto unlock;

  do {
    if (p == src->core_pick || p == src->curr) goto next;

    if (!is_cpu_allowed(p, this)) goto next;

    if (p->core_occupation > dst->idle->core_occupation) goto next;
...
static bool steal_cookie_task(int cpu, struct sched_domain *sd) {
  ...
  for_each_cpu_wrap(i, sched_domain_span(sd), cpu) {
    ...
    if (try_steal_cookie(cpu, i)) return true;
  }
...
static void sched_core_balance(struct rq *rq) {
  ...
  for_each_domain(cpu, sd) {
    ...
    if (steal_cookie_task(cpu, sd)) break;
  }
...
static void queue_core_balance(struct rq *rq) {
  if (!sched_core_enabled(rq)) return;

  if (!rq->core->core_cookie) return;
...
static void sched_core_cpu_starting(unsigned int cpu) {
  ...
  if (cpumask_weight(smt_mask) == 1) goto unlock;
...
static void __sched notrace __schedule(unsigned int sched_mode) {
  ...
  if (!(sched_mode & SM_MASK_PREEMPT) && prev_state) {
    if (signal_pending_state(prev_state, prev)) {
      ...
      if (prev->sched_contributes_to_load) rq->nr_uninterruptible++;
...
void __sched notrace dynamic_preempt_schedule(void) {
  if (!static_branch_unlikely(&sk_dynamic_preempt_schedule)) return;
  preempt_schedule();
}
...
void __sched notrace dynamic_preempt_schedule_notrace(void) {
  if (!static_branch_unlikely(&sk_dynamic_preempt_schedule_notrace)) return;
...
static inline int __rt_effective_prio(struct task_struct *pi_task, int prio) {
  if (pi_task) prio = min(prio, pi_task->prio);
...
void rt_mutex_setprio(struct task_struct *p, struct task_struct *pi_task) {
  ...
  if (oldprio == prio) queue_flag &= ~DEQUEUE_MOVE;

  prev_class = p->sched_class;
  queued = task_on_rq_queued(p);
  running = task_current(rq, p);
  if (queued) dequeue_task(rq, p, queue_flag);
  if (running) put_prev_task(rq, p);
   ...
   */
  if (dl_prio(prio)) {
    if (!dl_prio(p->normal_prio) || (pi_task && dl_prio(pi_task->prio) &&
    ...
    if (dl_prio(oldprio)) p->dl.pi_se = &p->dl;
    if (oldprio < prio) queue_flag |= ENQUEUE_HEAD;
  } else {
    if (dl_prio(oldprio)) p->dl.pi_se = &p->dl;
  ...
  if (queued) enqueue_task(rq, p, queue_flag);
  if (running) set_next_task(rq, p);
...
unsigned long effective_cpu_util(int cpu, unsigned long util_cfs,
  ...
  if (type == FREQUENCY_UTIL) util = uclamp_rq_util_with(rq, util, p);
   ...
   */
  if (type == ENERGY_UTIL) util += dl_util;
...
unsigned long sched_cpu_util(int cpu, unsigned long max) {
  return effective_cpu_util(cpu, cpu_util_cfs(cpu), max, ENERGY_UTIL, NULL);
}
#endif /* CONFIG_SMP */
...
static int user_check_sched_setscheduler(struct task_struct *p,
  ...
  if (task_has_idle_policy(p) && !idle_policy(policy)) {
    if (!is_nice_reduction(p, task_nice(p))) goto req_priv;
  }

  /* Can't change other user's priorities: */
  if (!check_same_owner(p)) goto req_priv;

  /* Normal users shall not reset the sched_reset_on_fork flag: */
  if (p->sched_reset_on_fork && !reset_on_fork) goto req_priv;

  return 0;

req_priv:
  if (!capable(CAP_SYS_NICE)) return -EPERM;
...
recheck:
  ...
  if (attr->sched_flags & ~(SCHED_FLAG_ALL | SCHED_FLAG_SUGOV)) return -EINVAL;
  ...
  if (unlikely(policy == p->policy)) {
    ...
    if (attr->sched_flags & SCHED_FLAG_UTIL_CLAMP) goto change;
...
change:
  ...
  if (pi) {
    ...
    if (newprio == oldprio) queue_flags &= ~DEQUEUE_MOVE;
  }

  queued = task_on_rq_queued(p);
  running = task_current(rq, p);
  if (queued) dequeue_task(rq, p, queue_flags);
  if (running) put_prev_task(rq, p);
...
SYSCALL_DEFINE3(sched_setattr, pid_t, pid, struct sched_attr __user *, uattr,
  ...
  if (likely(p)) {
    if (attr.sched_flags & SCHED_FLAG_KEEP_PARAMS) get_params(p, &attr);
...
static int sched_attr_copy_to_user(struct sched_attr __user *uattr,
  ...
  if (!access_ok(uattr, usize)) return -EFAULT;
  ...
  if (copy_to_user(uattr, kattr, kattr->size)) return -EFAULT;
...
SYSCALL_DEFINE4(sched_getattr, pid_t, pid, struct sched_attr __user *, uattr,
  ...
  if (!uattr || pid < 0 || usize > PAGE_SIZE || usize < SCHED_ATTR_SIZE_VER0 ||
      flags)
    return -EINVAL;
  ...
  if (p->sched_reset_on_fork) kattr.sched_flags |= SCHED_FLAG_RESET_ON_FORK;
  get_params(p, &kattr);
...
int dl_task_check_affinity(struct task_struct *p, const struct cpumask *mask) {
   ...
   */
  if (!task_has_dl_policy(p) || !dl_bandwidth_enabled()) return 0;
   ...
   */
  rcu_read_lock();
  if (!cpumask_subset(task_rq(p)->rd->span, mask)) ret = -EBUSY;
  rcu_read_unlock();
  return ret;
}
#endif
...
static int __sched_setaffinity(struct task_struct *p,
  ...
  if (!alloc_cpumask_var(&cpus_allowed, GFP_KERNEL)) return -ENOMEM;
...
int __sched dynamic_cond_resched(void) {
  if (!static_branch_unlikely(&sk_dynamic_cond_resched)) return 0;
...
again:
  ...
  if (task_running(p_rq, p) || !task_is_running(p)) goto out_unlock;
...
SYSCALL_DEFINE2(sched_rr_get_interval, pid_t, pid,
  ...
  if (retval == 0) retval = put_timespec64(&t, interval);
...
SYSCALL_DEFINE2(sched_rr_get_interval_time32, pid_t, pid,
  ...
  if (retval == 0) retval = put_old_timespec32(&t, interval);
  return retval;
}
#endif
...
void sched_show_task(struct task_struct *p) {
  ...
  if (!try_get_task_stack(p)) return;

  pr_info("task:%-15.15s state:%c", p->comm, task_state_to_char(p));

  if (task_is_running(p)) pr_cont("  running task    ");
...
static inline bool state_filter_match(unsigned long state_filter,
  ...
  if (!(state & state_filter)) return false;

  /*
   * When looking for TASK_UNINTERRUPTIBLE skip TASK_IDLE (allows
   * TASK_KILLABLE).
   */
  if (state_filter == TASK_UNINTERRUPTIBLE && state == TASK_IDLE) return false;
...
#ifdef CONFIG_SCHED_DEBUG
  if (!state_filter) sysrq_sched_debug_show();
#endif
  rcu_read_unlock();
  /*
...
int task_can_attach(struct task_struct *p,
  ...
  if (dl_task(p) &&
    ...
    if (unlikely(cpu >= nr_cpu_ids)) return -EINVAL;
...
static void print_preempt_disable_ip(int preempt_offset, unsigned long ip) {
  if (!IS_ENABLED(CONFIG_DEBUG_PREEMPT)) return;

  if (preempt_count() == preempt_offset) return;
...
void __might_resched(const char *file, int line, unsigned int offsets) {
  ...
  if (IS_ENABLED(CONFIG_PREEMPT_RCU)) {
    pr_err("RCU nest depth: %d, expected: %u\n", rcu_preempt_depth(),
           offsets >> MIGHT_RESCHED_RCU_SHIFT);
  }
...
void __cant_sleep(const char *file, int line, int preempt_offset) {
  ...
  if (!IS_ENABLED(CONFIG_PREEMPT_COUNT)) return;
...
void __cant_migrate(const char *file, int line) {
  ...
  if (is_migration_disabled(current)) return;
  ...
  pr_err(
      "in_atomic(): %d, irqs_disabled(): %d, migration_disabled() %u pid: %d, "
      "name: %s\n",
...
void sched_move_task(struct task_struct *tsk) {
  ...
  if (queued) dequeue_task(rq, tsk, queue_flags);
  if (running) put_prev_task(rq, tsk);

  sched_change_group(tsk, TASK_MOVE_GROUP);

  if (queued) enqueue_task(rq, tsk, queue_flags);
  if (running) {
...
static int cpu_cgroup_can_attach(struct cgroup_taskset *tset) {
  ...
  cgroup_taskset_for_each(task, css, tset) {
     ...
     */
    if (READ_ONCE(task->__state) == TASK_NEW) ret = -EINVAL;
...
static void cpu_util_update_eff(struct cgroup_subsys_state *css) {
  ...
  css_for_each_descendant_pre(css, top_css) {
    ...
    for_each_clamp_id(clamp_id) {
      if (eff[clamp_id] == uc_se[clamp_id].value) continue;
...
static int cpu_shares_write_u64(struct cgroup_subsys_state *css,
  ...
  if (shareval > scale_load_down(ULONG_MAX)) shareval = MAX_SHARES;
...
static int tg_set_cfs_bandwidth(struct task_group *tg, u64 period, u64 quota,
   ...
   */
  if (quota != RUNTIME_INF && quota > max_cfs_runtime) return -EINVAL;
...
static int tg_set_cfs_burst(struct task_group *tg, long cfs_burst_us) {
  ...
  if ((u64)cfs_burst_us > U64_MAX / NSEC_PER_USEC) return -EINVAL;
...
static int cpu_weight_write_u64(struct cgroup_subsys_state *css,
   ...
   */
  if (weight < CGROUP_WEIGHT_MIN || weight > CGROUP_WEIGHT_MAX) return -ERANGE;
...
static int cpu_weight_nice_write_s64(struct cgroup_subsys_state *css,
  ...
  if (nice < MIN_NICE || nice > MAX_NICE) return -ERANGE;
...
static int __maybe_unused cpu_period_quota_parse(char *buf, u64 *periodp,
  ...
  if (sscanf(buf, "%20s %llu", tok, periodp) < 1) return -EINVAL;
...
static ssize_t cpu_max_write(struct kernfs_open_file *of, char *buf,
  ...
  if (!ret) ret = tg_set_cfs_bandwidth(tg, period, quota, burst);
  return ret ?: nbytes;
}
#endif
