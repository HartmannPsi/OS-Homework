...
static void __exit_signal(struct task_struct *tsk) {
  ...
  if (group_dead) posix_cpu_timers_exit_group(tsk);
#endif
...
static void clean_kv_store(struct tsk_struct *p) {
  for (int i = 0; i < 1024; ++i) {
    ...
    spin_lock(&p->kv_lock[i]);
    hlist_for_each_entry_safe(entry, n, &p->kv_store[i], node) {
      hlist_del(&entry->node);
      kfree(entry);
    }
    spin_unlock(&p->kv_lock[i]);
  }
}
...
int rcuwait_wake_up(struct rcuwait *w) {
  ...
  if (task) ret = wake_up_process(task);
  rcu_read_unlock();
...
static bool is_effectively_child(struct wait_opts *wo, bool ptrace,
  ...
  struct task_struct *parent = !ptrace ? target->real_parent : target->parent;

  return current == parent ||
         (!(wo->wo_flags & __WNOTHREAD) && same_thread_group(current, parent));
}
...
static int do_wait_pid(struct wait_opts *wo) {
  ...
  if (target && target->ptrace && is_effectively_child(wo, ptrace, target)) {
    retval = wait_consider_task(wo, ptrace, target);
    if (retval) return retval;
  }
...
static long kernel_waitid(int which, pid_t upid, struct waitid_info *infop,
  ...
  ret = do_wait(&wo);
  if (!ret && !(options & WNOHANG) && (f_flags & O_NONBLOCK)) ret = -EAGAIN;
...
SYSCALL_DEFINE5(waitid, int, which, pid_t, upid, struct siginfo __user *, infop,
  ...
  if (!user_write_access_begin(infop, sizeof(*infop))) return -EFAULT;
...
int kernel_wait(pid_t pid, int *stat) {
  ...
  ret = do_wait(&wo);
  if (ret > 0 && wo.wo_stat) *stat = wo.wo_stat;
  put_pid(wo.wo_pid);
  return ret;
}
...
COMPAT_SYSCALL_DEFINE4(wait4, compat_pid_t, pid, compat_uint_t __user *,
  ...
  if (err > 0) {
    if (ru && put_compat_rusage(&r, ru)) return -EFAULT;
  }
  return err;
}
...
bool thread_group_exited(struct pid *pid) {
  ...
  exited = !task || (READ_ONCE(task->exit_state) && thread_group_empty(task));
  rcu_read_unlock();
