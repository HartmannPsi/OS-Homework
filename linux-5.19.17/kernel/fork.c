...
static bool try_release_thread_stack_to_cache(struct vm_struct *vm) {
  ...
  for (i = 0; i < NR_CACHED_STACKS; i++) {
    if (this_cpu_cmpxchg(cached_stacks[i], NULL, vm) != NULL) continue;
    return true;
  }
  return false;
}
...
static void thread_stack_free_rcu(struct rcu_head *rh) {
  ...
  if (try_release_thread_stack_to_cache(vm_stack->stack_vm_area)) return;
...
static int alloc_thread_stack_node(struct task_struct *tsk, int node) {
   ...
   */
  stack =
      __vmalloc_node_range(THREAD_SIZE, THREAD_ALIGN, VMALLOC_START,
                           VMALLOC_END, THREADINFO_GFP & ~__GFP_ACCOUNT,
...
void put_task_stack(struct task_struct *tsk) {
  if (refcount_dec_and_test(&tsk->stack_refcount)) release_task_stack(tsk);
}
#endif
...
void free_task(struct task_struct *tsk) {
  ...
  if (tsk->flags & PF_KTHREAD) free_kthread_struct(tsk);
...
static __latent_entropy int dup_mmap(struct mm_struct *mm,
  ...
  for (mpnt = oldmm->mmap; mpnt; mpnt = mpnt->vm_next) {
    ...
    if (retval) goto fail_nomem_anon_vma_fork;
    ...
    if (file) {
      ...
      if (tmp->vm_flags & VM_SHARED) mapping_allow_writable(mapping);
...
static inline void free_signal_struct(struct signal_struct *sig) {
   ...
   */
  if (sig->oom_mm) mmdrop_async(sig->oom_mm);
  kmem_cache_free(signal_cachep, sig);
}

static inline void put_signal_struct(struct signal_struct *sig) {
  if (refcount_dec_and_test(&sig->sigcnt)) free_signal_struct(sig);
}
...
#define ARCH_MIN_TASKALIGN 0
  ...
  task_struct_cachep = kmem_cache_create_usercopy(
      "task_struct", arch_task_struct_size, align, SLAB_PANIC | SLAB_ACCOUNT,
...
#endif
  if (orig->cpus_ptr == &orig->cpus_mask) tsk->cpus_ptr = &tsk->cpus_mask;
...
#ifdef CONFIG_MEMCG
  if (mm->owner == p) WRITE_ONCE(mm->owner, NULL);
#endif
}
...
int set_mm_exe_file(struct mm_struct *mm, struct file *new_exe_file) {
  ...
  if (new_exe_file) {
     ...
     */
    if (unlikely(deny_write_access(new_exe_file))) return -EACCES;
...
int replace_mm_exe_file(struct mm_struct *mm, struct file *new_exe_file) {
  ...
  if (old_exe_file) {
    ...
    for (vma = mm->mmap; vma && !ret; vma = vma->vm_next) {
      ...
      if (path_equal(&vma->vm_file->f_path, &old_exe_file->f_path))
        ret = -EBUSY;
    }
    mmap_read_unlock(mm);
    fput(old_exe_file);
...
struct file *get_task_exe_file(struct task_struct *task) {
  ...
  mm = task->mm;
  if (mm) {
    if (!(task->flags & PF_KTHREAD)) exe_file = get_mm_exe_file(mm);
  }
  task_unlock(task);
  return exe_file;
}
...
static int copy_sighand(unsigned long clone_flags, struct task_struct *tsk) {
  ...
  if (clone_flags & CLONE_CLEAR_SIGHAND) flush_signal_handlers(tsk, 0);
...
struct pid *pidfd_pid(const struct file *file) {
  if (file->f_op == &pidfd_fops) return file->private_data;
...
static __poll_t pidfd_poll(struct file *file, struct poll_table_struct *pts) {
   ...
   */
  if (thread_group_exited(pid)) poll_flags = EPOLLIN | EPOLLRDNORM;
...
static __latent_entropy struct task_struct *copy_process(
  ...
  if (clone_flags & (CLONE_THREAD | CLONE_VM)) {
    if (nsp->time_ns != nsp->time_ns_for_children) return ERR_PTR(-EINVAL);
  }
  ...
  if (clone_flags & CLONE_PIDFD) {
     ...
     */
    if (clone_flags & (CLONE_DETACHED | CLONE_THREAD)) return ERR_PTR(-EINVAL);
  }
  ...
  if (task_sigpending(current)) goto fork_out;

  retval = -ENOMEM;
  p = dup_task_struct(current, node);
  if (!p) goto fork_out;
  p->flags &= ~PF_KTHREAD;
  if (args->kthread) p->flags |= PF_KTHREAD;

  // init kv_store & kv_lock
  for (int i = 0; i < 1024; ++i) {
    INIT_HLIST_HEAD(&p->kv_store[i]);
    spin_lock_init(&p->kv_lock[i]);
  }
   ...
   */
  retval = -EAGAIN;
  if (data_race(nr_threads >= max_threads)) goto bad_fork_cleanup_count;
  ...
  cgroup_fork(p);
  if (args->kthread) {
    if (!set_kthread_struct(p)) goto bad_fork_cleanup_delayacct;
  }
#ifdef CONFIG_NUMA
   ...
   */
  if ((clone_flags & (CLONE_VM | CLONE_VFORK)) == CLONE_VM) sas_ss_reset(p);
...
noinline static int copy_clone_args_from_user(struct kernel_clone_args *kargs,
  ...
  if (unlikely(usize < CLONE_ARGS_SIZE_VER0)) return -EINVAL;

  err = copy_struct_from_user(&args, sizeof(args), uargs, usize);
  if (err) return err;

  if (unlikely(args.set_tid_size > MAX_PID_NS_LEVEL)) return -EINVAL;

  if (unlikely(!args.set_tid && args.set_tid_size > 0)) return -EINVAL;
  ...
  *kargs = (struct kernel_clone_args){
  ...
  if (args.set_tid && copy_from_user(kset_tid, u64_to_user_ptr(args.set_tid),
...
static inline bool clone3_stack_valid(struct kernel_clone_args *kargs) {
  if (kargs->stack == 0) {
    if (kargs->stack_size > 0) return false;
  } else {
    if (kargs->stack_size == 0) return false;
...
static bool clone3_args_valid(struct kernel_clone_args *kargs) {
   ...
   */
  if (kargs->flags & (CLONE_DETACHED | CSIGNAL)) return false;
  ...
  if (!clone3_stack_valid(kargs)) return false;
...
SYSCALL_DEFINE2(clone3, struct clone_args __user *, uargs, size_t, size) {
  ...
  if (!clone3_args_valid(&kargs)) return -EINVAL;
...
static int check_unshare_flags(unsigned long unshare_flags) {
  if (unshare_flags & ~(CLONE_THREAD | CLONE_FS | CLONE_NEWNS | CLONE_SIGHAND |
                        ...
                        CLONE_NEWPID | CLONE_NEWCGROUP | CLONE_NEWTIME))
    return -EINVAL;
  /*
