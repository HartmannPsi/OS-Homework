 ...
 * architecture-dependent parts of early kernel initialization.
 */
#include <asm/apic.h>
 ...
 *
 * The direct mapping only covers E820_TYPE_RAM regions, so the ranges and gaps
 * are represented by pfn_mapped[].
 */
...
static u64 __init get_ramdisk_image(void) {
  ...
  if (ramdisk_image == 0) ramdisk_image = phys_initrd_start;
...
static u64 __init get_ramdisk_size(void) {
  ...
  if (ramdisk_size == 0) ramdisk_size = phys_initrd_size;
...
static int __init reserve_crashkernel_low(void) {
  ...
  /* crashkernel=Y,low */
  ret =
      parse_crashkernel_low(boot_command_line, low_mem_limit, &low_size, &base);
  if (ret) {
    /*
  ...
  if (!low_base) {
    pr_err(
        "Cannot reserve %ldMB crashkernel low memory, please try smaller "
        "size.\n",
  ...
  pr_info(
      "Reserving %ldMB of low memory at %ldMB for crashkernel (low RAM limit: "
      "%ldMB)\n",
      (unsigned long)(low_size >> 20), (unsigned long)(low_base >> 20),
...
static void __init reserve_crashkernel(void) {
  ...
  if (ret != 0 || crash_size <= 0) {
    /* crashkernel=X,high */
    ret = parse_crashkernel_high(boot_command_line, total_mem, &crash_size,
                                 &crash_base);
  ...
  if (!crash_base) {
    ...
    if (!crash_base)
      crash_base = memblock_phys_alloc_range(crash_size, CRASH_ALIGN,
                                             CRASH_ALIGN, CRASH_ADDR_HIGH_MAX);
    if (!crash_base) {
  ...
  pr_info(
      "Reserving %ldMB of memory at %ldMB for crashkernel (System RAM: "
      "%ldMB)\n",
...
static bool __init snb_gfx_workaround_needed(void) {
  ...
  if (!early_pci_allowed()) return false;
  ...
  for (i = 0; i < ARRAY_SIZE(snb_ids); i++)
    if (devid == snb_ids[i]) return true;
#endif
...
static void __init early_reserve_memory(void) {
   ...
   */
  memblock_reserve(__pa_symbol(_text), (unsigned long)__end_of_kernel_reserve -
                                           (unsigned long)_text);
...
void __init setup_arch(char **cmdline_p) {
  ...
  if (!boot_params.hdr.root_flags) root_mountflags &= ~MS_RDONLY;
  ...
  *cmdline_p = command_line;
  ...
  if (efi_enabled(EFI_BOOT)) efi_memblock_x86_reserve_range();
   ...
   */
  if (movable_node_is_enabled()) memblock_set_bottom_up(true);
#endif
  ...
  if (efi_enabled(EFI_BOOT)) efi_init();
  ...
  if (mtrr_trim_uncached_memory(max_pfn)) max_pfn = e820__end_of_ram_pfn();
  ...
  if (init_ohci1394_dma_early) init_ohci1394_dma_on_all_controllers();
#endif
  ...
  if (!early_xdbc_setup_hardware()) early_xdbc_register_console();
...
#ifdef CONFIG_EFI
  if (efi_enabled(EFI_BOOT)) efi_apply_memmap_quirks();
#endif
